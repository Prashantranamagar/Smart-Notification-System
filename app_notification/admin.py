...
from .models import (
    EventType, NotificationPreference, UserEventPreference,
    ...
    def display_event_preferences(self, obj):
        ...
        return format_html(
            ...
            format_html_join(
                '\n',
                "<li><strong>{}</strong>: {}</li>",
                ((p.event_type.name, 'Enabled' if p.is_enabled else 'Disabled') for p in prefs)
            )
        )
...
class NotificationDeliveryInline(admin.TabularInline):
    ...
    extra = 0
    readonly_fields = ['attempted_at', 'delivered_at', 'failed_at', 'created_at', 'updated_at']

@admin.register(Notification)
class NotificationAdmin(admin.ModelAdmin):
    list_display = ['user', 'event_type', 'title', 'is_read', 'created_at', 'delivery_status']
    ...
    def delivery_status(self, obj):
        ...
        for status, count in status_counts.items():
            ...
            status_html.append(f'<span style="color: {color};">{status}: {count}</span>')
...
class NotificationDeliveryAdmin(admin.ModelAdmin):
    list_display = ['notification', 'channel', 'status', 'retry_count', 'attempted_at', 'delivered_at']
